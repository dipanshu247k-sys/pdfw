...
Run this in Windows CMD to download and extract latest poppler binaries into "%USERPROFILE%\Downloads\pdfw-tools":
...
python "%USERPROFILE%\Downloads\pdfw-tools\tools\pdfw.py" "C:\path\to\input.pdf" -wmark "C:\path\to\file.jpg"
```
...
When `-wmark` is provided, `pdfw.py` overlays the given image onto each output PDF page as a centered watermark at 75% transparency (25% opacity).
